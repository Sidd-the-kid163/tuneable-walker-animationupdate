function file = export(obj, varargin)
    ...
    % Use a fixed export path
    export_path = '/home/sidd163/Desktop/tunable-walker-trajopt-main/export/human_PF_adjustable/opt';
    
    % Make sure it exists
    if ~(exist(export_path, 'dir') == 7)
        error(['The path to export functions does not exist:\n%s\n'], export_path);
    end
    
    % Convert Windows '\' to '/' for Mathematica compatibility
    if ispc
        export_path = strrep(export_path, '\', '/');
    end
