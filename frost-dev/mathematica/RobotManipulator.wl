...
InertiaMatrixByLink[robotLink__,nDof_] :=
	Block[{MM, link, Je, De, i},
		...
		MM = SparseArray@Map[ExtraUtils`FrostBlockDiagonalMatrix[{I3*GetMass[#],GetInertia[#]}]&, link];
...
InertiaMatrix[robotLinks__,nDof_] :=
	Block[{MM, links, Je, De, i},
		...
		(* the mass/inertia matrix *)
		MM = Map[ExtraUtils`FrostBlockDiagonalMatrix[{I3*GetMass[#],GetInertia[#]}]&, links];
